...
describe.sequential("issue thread interaction routes", () => {
  ...
  it("cancels question interactions without waking the assignee", async () => {
    const app = await createApp();
    ...
    // A board-cancelled question carries no decision to act on. Waking here
    // meant a burst of cancellations spawned one concurrent session each.
    expect(mockHeartbeatService.wakeup).not.toHaveBeenCalled();
    expect(mockLogActivity).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({
