...
async function queueResolvedInteractionContinuationWakeup(input: {
  ...
  // A board-cancelled question carries no decision for the assignee to act
  // on, so cancelling it must not start a run. This is scoped to the
  // board-only /cancel action (ask_user_questions only) — /withdraw still
  // wakes the assignee on a cancelled status, since a third party withdrawing
  // a gate is itself something the assignee needs to know about. Without this
  // scoping, cancelling five stale gates in one burst spawned five concurrent
  // sessions on the same issue (MEL-56, 2026-08-07).
  if (
    input.interaction.status === "cancelled"
    && input.source === "issue.interaction.cancel"
    && !reviewPathLost
  ) return;
